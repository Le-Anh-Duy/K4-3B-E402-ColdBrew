...
| | |
|---|---|
| **Ai · việc gì** | Học viên AI20k làm quiz ôn xong, không biết phải học lại phần nào |
...
| **Lát cắt** | Quiz 5 câu → luật gom tín hiệu yếu → hỏi 3 câu nền → khoanh chỗ hổng (hoặc trả lời *chưa đủ căn cứ*) → nội dung cần ôn kèm mã đoạn transcript |
| **Quyết định AI** | Luật định vị, LLM chỉ diễn giải. AI không chọn nhánh, không chấm |
| **7 ngưỡng chốt CP4** | 2 đạt · 2 trượt (dẫn nguồn 80%/90% · hữu ích 55%/70%) · 3 chưa đo — §7.1 và §7.3 |
| **Chưa xong** | 0/20 bản chấm tay hợp lệ · quiz mới chạm 5/30 node · lớp lỗi 3 chưa có phép đo — danh sách đủ ở §7.5 |

Bằng chứng chi tiết: `eval/evidence/mining.md`, `validation/survey.md`. Kết quả đo: `eval/`.
...
| AI provider | Gemini qua endpoint tương thích OpenAI | SDK `openai`, `base_url=…/v1beta/openai/`, model `gemini-3.5-flash-lite`. Free tier **15 req/phút** nên mọi script gọi AI đều tiết lưu. Đổi provider chỉ sửa `base_url` + `MODEL` |
| Lưu trữ graph + learner state | JSON file | đủ cho 30 concept; không dựng graph DB |

Mô hình dữ liệu (node · cạnh · provenance · nhãn eval): `docs/data-model.md`. Luồng và quy tắc chẩn đoán: `mockup/flow.md`.

**Cây tri thức** (nhóm tự dựng; đề không cấp graph mẫu): 30 node **sinh bằng mô hình mạnh đọc `transcript-01-clean.md`** — *Day 2 sáng · Xác định bài toán kinh doanh cho AI*, 89 đoạn `[T01-001…089]` — nhóm rà lại và chốt, **chưa có người thứ hai đối chiếu độc lập**. Mỗi node mang `file` · `span` (mã đoạn) · `conf` (0,9 nếu nói thẳng trong đoạn, 0,7 nếu gom từ nhiều đoạn), và cạnh `prereq` tách riêng khỏi quan hệ mục lục. Slide chưa đối chiếu nên **không ghi số trang**; ràng buộc này đã đưa vào cả 5 file prompt của backend. Repo không chứa data pack, chỉ trích mã đoạn.
...
**Chuẩn B — mining chatlog VLearn.** `scripts/mining.py` (có `--selftest`) sinh ra `eval/evidence/mining.md`, nơi khai đủ luật đếm. Data pack không nằm trong repo (`DATA_NOTICE.md`); tải `tutor_turns.csv` rồi chạy lại sẽ ra đúng các số dưới đây. Quần thể sau lọc: **2.555 lượt · 384 học viên** (khoá K4, đã loại câu mẫu bấm sẵn của giao diện).
...
**Chuẩn A — khảo sát.** Form *"Làm sai rồi học gì tiếp?"*, **n = 31**, học viên trong khoá. Đủ 7 câu và phần hạn chế: [`validation/survey.md`](validation/survey.md).
...
1. **Công cụ hiện tại giảng mà không đo.** 87% lượt dùng một nước đi duy nhất là giảng lại, hỏi ngược 0,2%; hệ quả là 29% học viên quay lại cùng một mục ở dịp khác. Chỗ trống là bước **đo trước khi giảng** — đó là lát cắt §4.
...
3. **Nhấn định vị hơn là lộ trình.** Q2: *"Mình đang yếu kiến thức nào"* được 77,4% chọn, *"Nên học lại phần nào trước"* chỉ 38,7%. Lát cắt §4 gồm cả hai; nửa định vị có bằng chứng mạnh hơn. Đề C1 vẫn yêu cầu phần gợi ý ôn nên giữ, nhưng demo và slide nên mở bằng phần định vị kèm căn cứ.
...
| **C.** Chẩn đoán prerequisite từ quiz sai → nội dung cần ôn kèm giải thích | 29% quay lại cùng một mục (106/369) · khảo sát: 77,4% có làm sai trong 7 ngày | mỗi lần làm sai | **42% mất hơn 15 phút** tìm nguyên nhân (Q6), và 54,8% vẫn chưa hiểu sau chừng đó thời gian (Q5) | được | **✓** |

- **Loại A — thiếu bằng chứng, không phải thiếu giá trị.** Cả 13.494 lượt trong pack đều là học viên hỏi tutor; không có dữ liệu nào về giảng viên soạn quiz, nên không định lượng được cả ba cột đầu. Nhóm cũng không phải người dùng thật của nó.
- **Loại B — số mạnh nhưng lệch job.** 33% lớn hơn 29%, nhưng B chỉ sửa cách trình bày câu trả lời, không trả lời được câu [T10291] *"mình nên ôn phần nào trước?"*. Thêm nữa chỉ 0,3% lượt có rating, nên **không có cách đo B có hiệu quả hay không**.
- **Chọn C — mining và khảo sát chỉ vào cùng một chỗ.** Công cụ hiện tại giảng lại ở 87% lượt và hỏi ngược 0,2%; hệ quả đo được là 29% quay lại. C chèn vào đúng chỗ trống đó: đo trước, giảng sau. Khảo sát xác nhận từ phía học viên: thứ họ muốn nhất là *"mình đang yếu kiến thức nào"* (Q2, 77,4%), thứ cản họ nhiều nhất là *"không biết mình thiếu kiến thức nào"* (Q4, 54,8%). Khác B, C có tiêu chí thành công đo được ngay trong sản phẩm (§7).
...
| **ALEKS** (Knowledge Space Theory) | Môn học là đồ thị có hướng các kỹ năng với quan hệ tiền đề; assessment thích ứng dò ra "knowledge state", chia biết / chưa biết / **sẵn sàng học (outer fringe)** | "Outer fringe" tương ứng khái niệm **trần** (`ceiling`): node đã đạt là mép để học tiếp, không phải chỗ hổng | Cần **20–30 câu** mới định vị được | Chỉ có 5 câu và tối đa 9 câu nền. Đây là lý do tồn tại của luật "chưa đủ căn cứ" |
...
| **Khanmigo** (AI tutor) | LLM dạy kiểu Socratic, đọc được learning record của học viên | — | Dạy nhưng **không đo** — không có bước test để biết hổng ở đâu; và **không nhớ hôm trước học viên vướng gì**, trong khi chất lượng câu hỏi Socratic phụ thuộc vào điều đó | Luật định vị, LLM chỉ diễn giải — ngược lại |

Ba trong bốn phát hiện xác nhận thiết kế đang có: **ceiling** và **luật từ chối chẩn đoán** (ALEKS), **rule quyết định — LLM diễn giải** (Khanmigo). Hai điểm cần ghi nhận:

- **Lỗi của Khanmigo cũng là lỗi nhóm đang mắc.** "Các vòng không nối với nhau" chính là thứ `scripts/multiround.py` đo; điểm 4/5 người chấm ở M03 thuộc loại đó — một vấn đề cố hữu chứ không phải lỗi nhỏ.
- **Eedi chỉ ra một lỗ hổng chưa xử lý.** Distractor trong quiz hiện không mang thông tin nào, chỉ map *câu → node*; vì vậy phải hỏi thêm 3 câu nền mới suy ra được thứ Eedi lấy từ một lần bấm. Đây là nguồn tín hiệu rẻ nhất chưa khai thác — backlog §8.
...
- **Mức prototype nhắm tới:** [ ] Sketch [x] Mock → [ ] Working
  - **Thật:** chấm quiz · tín hiệu theo thời gian trả lời · suy luận prerequisite trên graph (rule, 3 bản đồng bộ) · provenance bằng mã đoạn · resume phiên · **một lời gọi AI thật vào quyết định trung tâm** (`/ai/plan/generate`).
  - **Mock:** nội dung của AI 1 trên trang dùng text soạn sẵn trong `mockup/data.js`; learner state lưu file JSON.
  - **Trạng thái CP4:** backend có 11 endpoint và 4 route AI, trang mock mới nối 1/4 — đúng mức **Mock** theo guide §3.2. Ba route còn lại là việc đấu dây, không phải feature mới.

- **Automation:** [x] augment  [ ] conditional  [ ] automate
  - Cost-of-error: chỉ dẫn sai khiến học viên ôn nhầm phần, mất thời gian và mất niềm tin. Vì vậy hệ thống **đề xuất kèm giải thích**, học viên và giảng viên thấy được căn cứ và bỏ qua được. Quyết định chọn nhánh do rule trên graph; LLM chỉ diễn giải, không tự suy ra prerequisite.

- **§4b. Nguyên tắc đã áp dụng (≥4):**

  | Nguyên tắc | Áp cụ thể vào đâu trong prototype |
  |---|---|
  | HAX G1 · *Make clear what the system can do* | Màn đầu nêu phạm vi: một bài giảng, cây 30 concept. Badge đầu trang tự dò backend, hiện "AI THẬT · &lt;model&gt;" hoặc "MOCK DATA · backend chưa chạy" |
...
Đây là giới hạn của **phép đo**, không phải của giao diện: chừng nào câu nền chưa bám đúng lá bị sai thì kết luận `y_le` vẫn còn dấu hỏi, kể cả khi ngưỡng 2 đạt 100%. Kéo theo một chiều đo chưa có: *câu nền sinh ra có thật sự liên quan tới lá bị sai không.*
...
**Kịch bản rủi ro nhất khi demo:** số 2 — AI trích một mã đoạn có thật nhưng thuộc phần khác. Nó đúng định dạng nên nhìn qua không phát hiện được; lỗi bịa hẳn còn dễ bắt hơn. Đây cũng là 3 trong 4 lượt trượt của ngưỡng 1.

**Độ phủ theo lớp.** Guide §2.6 yêu cầu ≥2 case mỗi lớp trong golden set:
...
- **Low-confidence (lớp 2):** không có câu sai nhưng có câu đúng mà chậm (>25s), hệ thống nêu rõ đang lấy thời gian trả lời làm tín hiệu rồi mới hỏi thêm; câu sai dưới 3s bị gắn cờ "có thể bấm bừa" thay vì coi là hổng chắc chắn.
- **Failure hoặc không căn cứ (lớp 1):** hai tầng chặn. *(a)* Luật không gom được tín hiệu về node nào, màn `Refuse` nêu lý do và không chẩn đoán bừa. *(b)* AI trả về nội dung trích sai phạm vi hoặc sai khung, `validate.py` bắt được; khi API lỗi hoặc bị giới hạn tốc độ thì trang hiện thẳng lý do rồi rơi về text mock, không trả nội dung rỗng trong im lặng.
...
- **Ngoài phạm vi (lớp 3):** phạm vi nêu ở màn đầu kèm badge nguồn dữ liệu, và `chat_prompts.py` quy tắc 3 cấm dùng kiến thức ngoài tư liệu được cấp. Nhưng chưa có case nào kiểm, và trang mock chưa nối endpoint chat — đường này hiện chỉ tồn tại ở backend (§7.5).
- **Đặc thù domain (lớp 4):** nhiều câu sai gom về node cha chung và chỉ chẩn đoán nhánh nhiều tín hiệu nhất; chạm gốc hoặc quá 3 vòng thì chuyển sang "học lại cả bài" kèm compact 3 ý.

## §7. Kiểm thử

### 7.1 Bảy ngưỡng — chốt tại CP4, không sửa sau mốc này

Nguyên tắc đặt ngưỡng: xác định **sai thì ai chịu hậu quả gì**, rồi neo con số vào một số liệu có thật (mining §1 · nghiên cứu §3 · ràng buộc kỹ thuật) — không nhìn kết quả rồi điều chỉnh cho vừa.

| # | Chiều | Định nghĩa kiểm chứng được | Đo bằng | Ngưỡng | Neo vào |
|---|---|---|---|---|---|
| 1 | **Dẫn nguồn đúng** | mọi mã đoạn AI trích ra có thật trong cây **và** nằm trong phần tư liệu đã cấp cho lượt đó | `grounding.py` | **≥90%** | Tutor hiện tại 33% câu trả lời không trích nguồn. Sản phẩm lấy provenance làm lõi (rubric C1: 25%) nên phải hơn hẳn chứ không chỉ ngang: giảm 33% xuống ≤10% |
| 2 | **Định vị đúng chỗ hổng** | node chọn để chẩn đoán **và** kết luận cuối (`scenario` + `gap` + `ceiling`) trùng nhãn nhóm gán tay | `run.py` | **≥90%** | ALEKS cần 20–30 câu trên cây hàng nghìn kỹ năng; ta có 14 câu trên cây 30 node, tức số câu hỏi trên mỗi node cao hơn nhiều. Bài toán nhỏ hơn nên không có cơ sở đặt ngưỡng thấp |
...
| 4 | **Hữu ích** | người chưa đọc tài liệu vẫn hiểu · giải thích phương án sai bằng nội dung · có hành động rõ | LLM mạnh hơn và người chấm | **≥70%** | Neo vào 29% quay lại. Lời khuyên dùng được ở ≥70% số lượt thì tỉ lệ phải quay lại không tệ hơn hiện trạng |
| 5 | **Nối được giữa các vòng** | người chấm xác nhận: vòng sau bám node vòng trước · không hỏi lại thứ đã hỏi · không kết luận hổng ở node vừa trả lời đúng | `review_rounds_ui.py` | **≥80% số phiên** | Đơn vị là cả phiên: `MAX_ROUNDS = 3` nên một phiên có 2 mối nối, 80% mỗi mối chỉ cho 0,8² = 64% phiên liền mạch. Tính theo vòng thì số đẹp mà phiên vẫn gãy |
...
| 7 | **Thời gian chờ** | từ lúc bấm đến lúc có nội dung AI | đồng hồ trong trang | trung vị **≤5s** · p90 **≤10s** | Tutor hiện tại trung vị 4,6s, p90 7,7s — học viên đã quen mức đó. *Lưu ý: `reply_ms` hai kỳ đo bằng hai nguồn khác nhau, chỉ so xu hướng* |

**Điều kiện cứng (đạt hoặc không, không phải ngưỡng %):** **0 lượt bịa mã đoạn không tồn tại.** Sai cách trình bày thì học viên còn kiểm lại được; bịa nguồn thì không, và 0,3% lượt có rating cho thấy không ai báo lỗi. Một lượt cũng là trượt.

Ba ngưỡng đang đặt **cao hơn kết quả đã biết** (1: 90% so với 80% · 4: 70% so với 55% · 6: 80% khi chưa có số nào). Nếu đặt cho vừa kết quả thì đã đặt 75% và 50%.

**Một giả định chưa có neo:** `SLOW_SEC = 25` và `RUSH_SEC = 3` là nhóm tự chọn. Data pack có thời gian tutor trả lời, không có thời gian học viên làm quiz. Hai con số này quyết định toàn bộ tín hiệu đầu vào của ngưỡng 2 và 3 nhưng không dựa trên dữ liệu nào — xem 7.5.
...
- **Chống trôi:** `run.py` kiểm vân tay bộ câu hỏi (node và đáp án). Đổi quiz mà quên gán nhãn lại thì bộ đo dừng và báo lỗi thay vì báo đạt.
...
Hai ngưỡng trượt (1 và 4), ba ngưỡng chưa đo (5 chưa tính được, 6, 7). Theo guide §4.1, không đạt quality bar nhưng phân tích được nguyên nhân vẫn tính đủ điểm — phân tích ở 7.4.
...
R0 để lộ 3 lỗ hổng — C07 thiếu cạnh `prereq`, C09 và C10 chưa biết từ chối chẩn đoán — cả ba đã sửa (§9). Con số 100% là kết quả trên **chính bộ case nhóm tự soạn**: nó chứng minh luật khớp đặc tả của nhóm, không chứng minh đặc tả đúng. Đó là việc của ngưỡng 6.
...
Bốn lượt trượt: 3 lượt trích mã đoạn ngoài phạm vi tư liệu được cấp, cả ba rơi vào kịch bản "học lại cả bài" nơi tư liệu cấp rộng nhất; 1 lượt câu tự kiểm sai định dạng. Không lượt nào bịa mã đoạn không tồn tại nên điều kiện cứng vẫn giữ. Lỗi dồn vào một kịch bản chứ không rải đều, tức đây là lỗi **phạm vi tư liệu cấp cho prompt** chứ không phải mô hình bịa. Hướng sửa sau CP4: thu hẹp tư liệu cấp cho kịch bản `nen_bai`.
...
Hai ngưỡng này đánh đổi nhau: ngưỡng 1 và điều kiện cứng đẩy prompt về phía **trích dẫn**, ngưỡng 4 kéo về phía **giảng giải**. Nới ràng buộc để đạt ngưỡng 4 gần như chắc chắn kéo ngưỡng 1 xuống — đó là bài toán thật của sản phẩm.
...
| **Trang mock chỉ nối 1/4 route AI** | Chỉ `/ai/plan/generate` là AI thật; `/ai/explain/*`, `/ai/diagnosis/*`, `/ai/chat/*` có ở backend nhưng trang không gọi | Ngưỡng 7 chưa đo được trên đường AI 1. §4 vì vậy khai mức Mock |
| **Quiz chỉ chạm 5/30 node** | 5 câu map vào 5 lá dưới 3 node cha; 25 node còn lại không có đường nào chạm tới | Trần trên của ngưỡng 2 và 3: 100% là 100% *trong phạm vi 5 lá đó*. Đang mở rộng ngân hàng câu hỏi ở backend |
| **Cây do mô hình sinh, một người chốt** | 30 node · 5 cạnh `prereq` · `conf` (26 node 0,9 · 6 node 0,7) chưa qua người thứ hai đối chiếu với transcript | Provenance (rubric 25%) dựa trên phép ánh xạ node sang mã đoạn chưa kiểm chéo |
| **`SLOW_SEC=25` · `RUSH_SEC=3`** | Nhóm tự chọn, không có dữ liệu nào đối chiếu — data pack không ghi thời gian học viên làm quiz | Quyết định toàn bộ tín hiệu đầu vào của ngưỡng 2 và 3. Hai số này sai thì con số 100% mất ý nghĩa |
| **Log khảo sát** | Đã khảo sát n=31, đạt ngưỡng ≥20 người, nhưng repo mới có bản tổng hợp biểu đồ | Chuẩn A chưa trọn vẹn; cần xuất CSV của Google Form vào `validation/` |
...
| **4 case `xfail` C23–C26** | Chưa build: engine không mang lịch sử vòng · trạng thái sau retest chưa vào phần tư vấn · prompt không nhận từng câu nền đã sai · resume chưa có case kiểm | Không tính vào ngưỡng 2; backlog §8 |
| **Sinh câu hỏi nền có điều kiện** | Chưa làm (§4). `PROBES` là bộ câu tĩnh chung cho cả node cha | Làm ngưỡng 2 và 3 kém chắc chắn hơn con số 100% thể hiện |
| **Case từ chatlog thật** | Guide §2.6 yêu cầu ≥10/20 case lấy hoặc phát triển từ chatlog thật; cả 26 case hiện tại đều do nhóm tự dựng | Bộ case chưa đạt cơ cấu guide yêu cầu |
...
- Backlog (trỏ về §3, §5, §7.5): gán misconception cho distractor · sinh câu hỏi nền có điều kiện · case cho lớp 3 · mở rộng ngân hàng câu hỏi ngoài 5 node lá.
...
| 18/9 | Chạy golden set R0: 17/20, lộ 3 lỗ hổng C07/C09/C10 | `eval/results.md` |
| 18/9 | Dựng lại cây từ `transcript-01-clean.md`: 30 node, provenance là mã đoạn `[T01-NNN]`, có `conf` | Cây cũ ghi số trang slide chưa đối chiếu; trích dẫn bịa là rủi ro nặng nhất của đề (25% provenance) |
| 18/9 | Thêm cạnh `prereq` tách khỏi quan hệ mục lục | Đề phân biệt `prerequisite` với `broader/narrower`; lát cắt nói "prerequisite" nên không lấy mục lục thay thế |
| 18/9 | Luật từ chối chẩn đoán khi tín hiệu toàn `rush` hoặc `slow` tản mát | Đoán bừa tệ hơn nói "chưa đủ căn cứ"; cũng là yêu cầu uncertainty rõ ràng của đề |
...
| 18/9 | Nối Gemini qua backend; đo 20 lượt: 16/20 có căn cứ, 11/20 hữu ích | Số đo đầu tiên có AI thật. Có căn cứ không đồng nghĩa dạy được |
| 18/9 | Thêm suite chuỗi nhiều vòng, 5 phiên | Trước đó không phép đo nào chạm tới `/ai/explain/round`, tức cơ chế trung tâm chưa từng được đo |
| 18/9 | Bản chấm tay gắn theo hash câu trả lời | Nhận xét cho bản cũ dính vào bản mới thì số đo thành sai |
| 18/9 | **Đếm lại toàn bộ evidence §1 bằng `scripts/mining.py`.** *65% hỏi lại cùng một phần* thành **29%**; *11% quay lại buổi trước* bị bỏ; *15% câu hỏi ôn hoặc học lại* thành **2,3%** và hạ xuống mức minh hoạ | Số cũ đếm theo tin nhắn (7,3 tin mỗi người nên hầu như ai cũng đạt "≥2 tin") và không loại câu mẫu. Bản cũ cũng không có script trong repo nên không kiểm lại được |
| 18/9 | Thêm khảo sát n=31 (chuẩn A) vào §1 | Mining chứng minh pain tồn tại; khảo sát chứng minh học viên muốn nó được giải |
| 18/9 | Bỏ mệnh đề "chỗ hổng thường nằm ở prerequisite" khỏi §1 | Không có bằng chứng. Sản phẩm **kiểm** giả thuyết đó cho từng người và có ba kết luận phủ định (§4) |
| 18/9 | Bốn file prompt đang yêu cầu AI trích số trang slide, đổi sang mã đoạn `[T01-NNN]` | §0 định nghĩa số trang slide là trích dẫn bịa; ví dụ mẫu trong prompt đang dẫn mô hình làm đúng điều đó |
| 18/9 | Hạ mức prototype từ Working xuống Mock | Trang mock mới nối 1/4 route AI |
| 18/9 | Chốt 7 ngưỡng tại CP4, mỗi ngưỡng neo vào một số liệu có thật | Yêu cầu của CP4: chốt "thế nào là đạt" trước khi biết kết quả |
