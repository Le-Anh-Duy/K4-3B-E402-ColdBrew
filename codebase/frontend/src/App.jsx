...
export default function App() {
  ...
  function navigate(nextPage) {
    setPage(nextPage === 'learning' && !session ? 'home' : nextPage)
  }

  async function start(config) {
    const nextSession = await learningService.startQuiz(config)
  ...
  return (
    <AppLayout user={user} page={page} onNavigate={navigate} onLogout={logout}>
      {page === 'home' && (
        <HomeScreen
          user={user}
          history={history}
          ...
          onResume={() => setPage('learning')}
        />
      )}
      {page === 'progress' && <ProgressScreen history={history} onHome={() => setPage('home')} />}
      {session && (
        <div hidden={page !== 'learning'}>
          <LearningScreen
            key={session.id}
            session={session}
            isActive={page === 'learning'}
            onComplete={finish}
            onHome={() => setPage('home')}
          />
        </div>
